...
describe("buildContrastPreview", () => {
  it("returns null without enough color data", () => {
    ...
    expect(buildContrastPreview({ ...m, fixed: null }, "unchecked", 1)).toBeNull();
  });

  it("located element re-audited & cleared → verified confidence with the real pair", () => {
  ...
  it("located element not re-audited but numbers pass → calculated (no verified claim)", () => {
    const p = buildContrastPreview(m, "unchecked", 1)!;
    expect(p.confidence).toBe("calculated");
  });

  it("located element re-audit failed → inconclusive with a background reason", () => {
  ...
  it("background-change fixes swap the simulated background, not the text", () => {
    const p = buildContrastPreview({ ...m, prop: "background", toHex: "#123456" }, "unchecked", 1)!;

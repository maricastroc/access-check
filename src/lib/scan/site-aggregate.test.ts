...
describe("aggregateScore", () => {
  it("returns null when no page has completed", () => {
  ...
  it("averages only the completed pages, rounding", () => {
    expect(aggregateScore([p("done", 90), p("done", 80)])).toBe(85);
    expect(aggregateScore([p("done", 90), p("done", 81)])).toBe(86);
  });

  it("ignores non-completed pages and those without a score in the calculation", () => {
